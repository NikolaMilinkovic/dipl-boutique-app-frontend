...
export interface DressStockDataDecrease {
  dressId: string;
  colorId: string;
  sizeId: string;
  increment: number;
              ...
              return {
                ...
                sizes: color.sizes.map((size) => {
                  if (size._id.toString() === data.sizeId.toString()) {
                    return { ...size, stock: Number(size.stock) - 1 };
                  }
                  return size;
                }),
              };
...
export interface PurseStockDataDecrease {
  purseId: string;
  colorId: string;
  decrement: number;
}
          ...
          colors: purse.colors.map((color) => {
            if (color._id.toString() === data.colorId.toString()) {
              return {
                ...color,
                stock:
                  color.stock - data.decrement >= 0
                    ? color.stock - data.decrement
                    : color.stock,
              };
            }
            return color;
...
export const increaseDressBatchStock = (
  ...
  setActiveDresses((prevDresses) => {
    ...
    return prevDresses.map((dress) => {
      const itemIndex = remainingData.findIndex(
        (d) => d.dressId.toString() === dress._id.toString(),
      );
      if (itemIndex !== -1) {
        ...
        return {
          ...
          colors: dress.colors.map((color) => {
            if (
              color._id.toString() === itemData.colorId.toString() &&
              color.sizes
            ) {
              return {
                ...color,
...
export const increasePurseBatchStock = (
  ...
  setActivePurses((prevPurses) => {
    ...
    return prevPurses.map((purse) => {
      const itemIndex = remainingData.findIndex(
        (d) => d.purseId.toString() === purse._id.toString(),
      );
...
export interface PurseStockDataIncrease {
  purseId: string;
  colorId: string;
  increment: number;
}
export interface DressStockDataIncrease {
  dressId: string;
  colorId: string;
  sizeId: string;
  increment: number;
}
export interface DataArrPropTypes {
